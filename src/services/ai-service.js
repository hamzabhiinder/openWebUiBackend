...
Only respond with the JSON object, no additional text.`
    ...
    /**
     * Generate actual PowerPoint file from confirmed structure
     * @param {object} pptStructure - The confirmed PPT structure
     * @param {string} provider - AI provider to use (for image generation if needed)
     * @returns {Promise<object>} - Generated PPT file information
     */
    async generatePPTFromStructure(pptStructure, provider = "OpenAI") {
        try {
            const ppt = new PptxGenJS();
            ...
            for (const [index, slideData] of pptStructure.slides.entries()) {
                ...
                if (slideData.type === 'title') {
                    ...
                    if (slideData.imagePrompt) {
                        console.log(`🖼️ Generating image for slide: "${slideData.title}"`);
                        const imageB64 = await this.generateImage(slideData.imagePrompt, provider);
                        if (imageB64) {
                            // Add image to PPTX from base64
                            slide.addImage({
                                data: `data:image/png;base64,${imageB64}`,
            ...
            console.log('✅ PPT file generated successfully:', filename);
            ...
            console.error('❌ Error generating PPT file:', error);
            throw error;
        }
    }
